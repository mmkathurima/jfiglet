...
enum SmushingRule implements Comparable<SmushingRule> {
    ...
    HORIZONTAL_UNDERSCORE_SMUSHING(Type.HORIZONTAL, Layout.CONTROLLED_SMUSHING, 2) {
        ...
        Character smush(char char1, char char2, char hardblank) {
            final String chars = "|/\\[]{}()<>";
            if (char1 == '_' && chars.indexOf(char2) != -1) return char2;
            else if (char2 == '_' && chars.indexOf(char1) != -1) return char1;
            else return null;
        }
    },
    ...
    HORIZONTAL_HIERARCHY_SMUSHING(Type.HORIZONTAL, Layout.CONTROLLED_SMUSHING, 4) {
        ...
        Character smush(char char1, char char2, char hardblank) {
            ...
            if (pos1 != -1 && pos2 != -1) if (pos1 != pos2 && Math.abs(pos1 - pos2) != 1)
    ...
    HORIZONTAL_OPPOSITE_PAIR_SMUSHING(Type.HORIZONTAL, Layout.CONTROLLED_SMUSHING, 8) {
        ...
        Character smush(char char1, char char2, char hardblank) {
            ...
            if (pos1 != -1 && pos2 != -1) if (pos1 != pos2 && Math.abs(pos1 - pos2) == 1) return '|';
            return null;
        }
    },
    ...
    HORIZONTAL_BIG_X_SMUSHING(Type.HORIZONTAL, Layout.CONTROLLED_SMUSHING, 16) {
        @Override
        Character smush(char char1, char char2, char hardblank) {
            if (char1 == '/' && char2 == "\\".charAt(0)) return '|';
            else if (char1 == "\\".charAt(0) && char2 == '/') return 'Y';
            else if (char1 == '>' && char2 == '<') return 'X';
            else return null;
        }
    },
    ...
    static {
        ...
        for (SmushingRule rule : SmushingRule.values()) results.put(rule.getCodeValue(), rule);
