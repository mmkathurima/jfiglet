...
public class FigletFont {
    ...
    public String getCharLineString(int c, int l) {
        if (font[c][l] == null)
            return null;
        else return new String(font[c][l]).replace(hardblank, ' ');
    }
    ...
    public FigletFont(InputStream stream) throws IOException {
        ...
        try (BufferedReader data = new BufferedReader(
            ...
            int j = 0;
            for (int c = 32; c <= 126; ++c) charsTo[j++] = c;
            for (int additional : new int[]{196, 214, 220, 228, 246, 252, 223}) charsTo[j++] = additional;
            ...
            while (dummyS != null) {  // for all the characters
                //System.out.print(i+":");
                if (charPos < REGULAR_CHARS) charCode = charsTo[charPos++];
                else {
                    dummyS = data.readLine();
                    if (dummyS == null)
                        continue;
    ...
    int convertCharCode(String input) {
        ...
        if (codeTag.matches("^0[xX][0-9a-fA-F]+$")) return Integer.parseInt(codeTag.substring(2), 16);
        else if (codeTag.matches("^0[0-7]+$")) return Integer.parseInt(codeTag.substring(1), 8);
        else return Integer.parseInt(codeTag);
    }
